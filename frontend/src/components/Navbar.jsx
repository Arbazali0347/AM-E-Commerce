      ...
      <div className="flex items-center justify-between px-5 md:px-10 py-4 font-medium">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative">
            <img
              onClick={() => {
                if (token) setShowDropdown(!showDropdown);
                else navigate("/login");
              }}
            ...
            />
            {token && showDropdown && (
              <div className="absolute right-0 top-full mt-2 flex flex-col gap-2 w-40 py-3 px-5 bg-white shadow-lg rounded-lg text-gray-600">
                <p
                ...
                <p
                  onClick={() => { logoutHandler(); setShowDropdown(false); }}
                  className="cursor-pointer hover:text-black"
                >
                  Logout
                </p>
          ...
          <Link to="/cart" className="relative">
            <img src={assets.cart_icon} alt="Cart" className="w-6 min-w-6 hover:scale-110 transition" />
            <p className="absolute right-[-6px] bottom-[-6px] w-5 h-5 flex items-center justify-center bg-white text-black rounded-full text-[10px]">
              {getCartCount()}
            </p>
          </Link>
